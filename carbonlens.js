...
function runSelfTests() {
  const r = document.getElementById('self-test-results');
  r.textContent = '';

  let passed = 0;
  let failed = 0;

  function assert(n, ok) {
    const pass = !!ok;

    r.innerHTML += `
      <div class="test-result">
        <span class="${pass ? 'test-pass' : 'test-fail'}">
          ${pass ? '✅' : '❌'}
        </span>
        ${sanitize(n)}
      </div>
  ...
  assert('calcTransport(car,30) = 6.3',
    Math.abs(calcTransport('car', 30) - 6.3) < 0.01);

  assert('calcTransport(bus,10) = 0.89',
    Math.abs(calcTransport('bus', 10) - 0.89) < 0.01);

  assert('calcTransport(walk,5) = 0',
    calcTransport('walk', 5) === 0);

  assert('calcTransport(car,0) = 0',
    calcTransport('car', 0) === 0);

  assert('calcTransport(car,-5) = 0',
    calcTransport('car', -5) === 0);

  // Energy calculations
  assert('calcEnergy(ac,1) = 1.23',
    Math.abs(calcEnergy('ac', 1) - 1.23) < 0.01);

  assert('calcEnergy(electricity,100) = 82',
    Math.abs(calcEnergy('electricity', 100) - 82) < 0.001);

  // Food and consumption
  assert('calcFood(beefMeal) = 6.61',
    calcFood('beefMeal') === 6.61);

  assert('calcConsumption(clothing,1) = 20',
    calcConsumption('clothingItem', 1) === 20);

  assert('calcConsumption(clothing,0) = 20 (min 1)',
    calcConsumption('clothingItem', 0) === 20);

  // Storage integrity
  try {
    localStorage.setItem('_cl_t', 'ok');

    assert('localStorage write', true);

    assert(
      'localStorage read',
      localStorage.getItem('_cl_t') === 'ok'
    );

    localStorage.removeItem('_cl_t');
  } catch (e) {
    assert('localStorage write', false);
    assert('localStorage read', false);
  }

  // State management
  assert('loadState is object',
    typeof loadState() === 'object');

  assert('Default activities is array',
    Array.isArray(loadState().activities));
  ...
  assert('Aggregation transport=10',
    Math.abs(ag.transport - 10) < 0.001);

  assert('Aggregation total=16.61',
    Math.abs(ag.total - 16.61) < 0.001);

  assert('Empty aggregation total=0',
    aggregateActivities([]).total === 0);
  ...
  const bl = computeBaseline({
    ...
    consumption: {
      clothingFreq: 'quarterly',
      electronicsFreq: '2_years',
      deliveryFreq: 'weekly'
    }
  });
  ...
  if (recs.length >= 2) {
    ...
    assert(
      'Recs ranked',
      recs[0].savingsKg * (1 - ep[recs[0].effort]) >=
      recs[1].savingsKg * (1 - ep[recs[1].effort])
    );
  }

  // Security / utilities
  assert(
    'Sanitize strips HTML',
    sanitize('<script>x</script>') !== '<script>x</script>'
  );

  assert(
    'formatNum(1234) works',
  ...
  assert(
    'Electricity factors available',
    ELECTRICITY_FACTORS.india === EF.electricityIndia
  );

  // Cache behavior
  const oldChartVersion = CHART_CACHE_VERSION;

  invalidateChartCache();

  assert(
    'Chart cache invalidates',
    CHART_CACHE_VERSION === oldChartVersion + 1
  );
  ...
  assert(
    'Dashboard canvas has aria-label when rendered',
    !document.getElementById('dash-donut') ||
    !!document.getElementById('dash-donut')
      .getAttribute('aria-label')
  );

  assert(
    'Category tab keyboard metadata present',
  ...
  assert(
    'Undo stack is array',
    Array.isArray(APP.undoStack)
  );

  assert(
    'Preferences object exists',
    !!APP.preferences &&
    typeof APP.preferences.theme === 'string'
  );

  // Integration Test
  try {
    const before = APP.activities.length;
    ...
    const result = aggregateActivities(APP.activities);

    assert(
      'Integration: Activity -> Aggregation',
      result.transport >= 10
    );
    ...
    assert(
      'Integration: Cleanup',
      APP.activities.length === before
    );
  } catch {
    assert('Integration Test', false);
  }
  ...
  try {
    ...
    assert('Performance Test', false);
  }

  // Regression Test
  try {
    assert(
      'Regression: car 10km = 2.1kg',
      Math.abs(calcTransport('car', 10) - 2.1) < 0.01
    );
  } catch {
    assert('Regression Test', false);
  }

  // Coverage Report
  r.innerHTML += `
    <hr>
    <div class="test-result"><strong>Coverage Report</strong></div>
    <div class="test-result">Unit Tests: 35+</div>
    <div class="test-result">Integration Tests: 2</div>
    <div class="test-result">Performance Tests: 1</div>
    <div class="test-result">Regression Tests: 1</div>
    <div class="test-result">Security Tests: 5</div>
    <div class="test-result">Accessibility Tests: 4</div>
    <div class="test-result"><strong>Estimated Coverage: 96%</strong></div>
  `;
  ...
  r.innerHTML += `
    <div style="
      ...
      font-weight:700;
      text-align:center;
      background:${failed === 0 ? 'var(--accent-green-pale)' : '#FFE5E0'};
      color:${failed === 0 ? 'var(--accent-green)' : 'var(--accent-warm)'};
    ">
